function [corr,resptms] = vibrato_detect_trial(stim,Fs,vibtimes,varargin)
...
%heard any vibrato in the audio.  The audio contains multiple vibratos, so
...
%array of vibrato start times that is delivered into this function from the
%vibrato insertion function.
%Robert Crews & Nate Zuk (2017-2018)

    %stim = audio
    %Fs = sampling rate (in Hz)
    %vibtimes = times of vibratos (in s)
...
corr = false(length(vibtimes),1);
for ii = 1:length(vibtimes), % for each vibrato time
    % check the number of key presses that were a correct detection of the
    % vibrato time
    detected = sum(resptms>=vibtimes(ii) & resptms<=vibtimes(ii)+detecttol);
